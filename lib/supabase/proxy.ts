...
export async function updateSession(request: NextRequest) {
  ...
  const ownHosts = ['localhost', 'linezheets.com', 'www.linezheets.com', 'linezheets.vercel.app'];
  const isOwnHost = ownHosts.some(h => host === h || host.startsWith(`${h}:`));
  ...
  const protectedPaths = ['/dashboard', '/inquiries'];
  ...
  if (user && (request.nextUrl.pathname === '/login' || request.nextUrl.pathname === '/join')) {
    const url = request.nextUrl.clone();
