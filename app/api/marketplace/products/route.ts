...
export async function GET(req: NextRequest) {
  ...
  const sortMap: Record<string, { col: string; asc: boolean }> = {
    ...
    price_asc  : { col: 'wsp_usd',    asc: true  },
    price_desc : { col: 'wsp_usd',    asc: false },
  ...
  if (minPrice)  q = q.gte('wsp_usd', Number(minPrice));
  if (maxPrice)  q = q.lte('wsp_usd', Number(maxPrice));
  if (search) {
  ...
  return NextResponse.json({ products: data ?? [], total: count ?? 0, offset, limit });
}
